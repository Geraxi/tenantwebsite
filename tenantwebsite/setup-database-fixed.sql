-- Complete Database Setup for Tenant CRM (FIXED VERSION)
-- Run this entire file in Supabase SQL Editor
-- This version has fixed RLS policies to prevent infinite recursion
...
-- Create a helper function to get user's agency_id (prevents recursion)
CREATE OR REPLACE FUNCTION auth.user_agency_id()
RETURNS UUID AS $$
  SELECT agency_id FROM users WHERE id = auth.uid() LIMIT 1;
$$ LANGUAGE sql SECURITY DEFINER STABLE;

-- RLS Policies for users (simplified to avoid recursion)
